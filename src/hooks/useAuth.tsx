...
import type { Users } from "@/types/me.type";
...
export default function useAuth() {
	...
	const loginWithProvider: AuthContextValue["loginWithProvider"] = async (
		provider: string,
	) => {
		try {
			setIsPending(true);
	...
	const resetPassword = async (password: string) => {
		try {
			...
			const data = await response.json();
			if (
				data.error === "New password should be different from the old password."
			) {
				setIsPending(false);
