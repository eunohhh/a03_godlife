...
function LogInForm() {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!email || !password) return showAlert("error", "이메일, 비밀번호를 모두 입력해주세요");
    ...
    const handleRecoverPassword = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // console.log(email);
        if (!email) return showAlert("error", "빈 값이 없도록 해주세요");
        if (/\s/.test(email)) return showAlert("error", "공백을 포함할 수 없습니다!");
        if (!emailRegex.test(email)) return showAlert("error", "유효한 이메일 주소를 입력하세요!");
